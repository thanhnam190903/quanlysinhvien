...
public class StudentScoreController {
    ...
    public String show(Principal principal, Model model){
        ...
        List<ScoreSubject> allScores = scoreSubjectRepository.findByStudentId(student.getId());
        List<Cycle> cycles = cycleRepository.findCyclesByStudent(student.getId());
        Long totalSubjects = scoreSubjectRepository.countAllSubjects(student.getId());
        Double overallAvg = scoreSubjectRepository.getOverallAverage(student.getId());
        ...
        for (Cycle cycle : cycles) {
            List<ScoreSubject> cycleScores = scoreSubjectRepository
                    .findByStudentAndCycle(student.getId(), cycle.getId());
            scoresByCycle.put(cycle.getId(), cycleScores);

            Double avg10 = scoreSubjectRepository
                    .getAverageScoreByCycle(student.getId(), cycle.getId());
            ...
            Long count = scoreSubjectRepository.countByCycle(student.getId(), cycle.getId());
            subjectCountByCycle.put(cycle.getId(), count);

            Long regradeCount = regradeRequestRepository
                    .countByCycle(student.getId(), cycle.getId());
        ...
        for (ScoreSubject score : allScores) {
            Long regradeExists = regradeRequestRepository
                    .checkRegradeExists(student.getId(), score.getSubject().getId());
